...
- 可选启动本地 MCP HTTP 服务，让本机 MCP 客户端直接访问当前 Obsidian vault
...
插件可以在 Obsidian 桌面端启动一个本地 MCP HTTP 服务。服务只监听 `127.0.0.1`，并要求所有 `/mcp` 请求携带 bearer token。这个就是 `obsidian-connect` / Codex 侧应该使用的插件端 endpoint。
