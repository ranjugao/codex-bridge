...
一个轻量的 Obsidian 桌面端插件，主要通过本地 MCP 服务把当前 vault 暴露给 Codex，让 Codex 可以搜索、读取和写入笔记。它也保留本地文件桥接命令，并支持把当前笔记内容发送到 OpenAI 或 OpenAI-compatible API 生成结构化 Markdown 摘要。

**标签：** `obsidian-plugin` `mcp` `codex` `local-first` `markdown` `notes` `ai-summary`
...
- 把 Codex 回复追加回当前笔记
- 把最新 Codex 回复创建为新笔记
- 将选中文本或整篇笔记发送到 AI API，总结到 `Codex/YYYY-MM-DD.md`
...
6. 关闭 Safe mode 后启用 `Codex Bridge`。
...
打开 Obsidian 命令面板，搜索 `Codex Bridge`。

常用命令：

- `Export active note context`
  - 把当前笔记上下文导出到 `_codex_bridge/context/`
- `Create Codex request from active note`
  - 基于当前笔记创建一个请求文件，方便复制给 Codex
- `Append latest bridge reply to active note`
  - 把 `_codex_bridge/replies/` 里的最新回复追加到当前笔记
...
- `AI summarize selection/current note to daily note`
  ...
  - 写入路径：`Codex/YYYY-MM-DD.md`
  - 文件存在时 append，不存在时 create

## 语言切换

进入 `Settings -> Codex Bridge`，在 `语言 / Language` 中选择：
...
- Obsidian 端安装本插件
- Codex 端安装 `obsidian-connect` skill
- 本插件负责暴露当前 vault 的本地 MCP endpoint
- `obsidian-connect` skill 负责让本机 Codex 通过 MCP 搜索、读取和写入笔记

这个本地 MCP 服务不会让托管式或移动端 AI 客户端自动访问你的电脑；`127.0.0.1` 只对本机可见。

进入 `Settings -> Codex Bridge`：
...
| `save_chat_summary` | 把聊天总结追加到 `Codex/YYYY-MM-DD.md` |
...
你可以把 Codex 的回复保存到 `replies/`，再用命令导入回当前笔记或创建新笔记。
...
发布时需要创建和 `manifest.json` 版本一致的 GitHub Release，例如 `0.6.0`，并把以下文件作为 release assets 上传：
